...
- **精確的預覽。** 需要核准的變更會先準備成預覽：app 列出每一筆紀錄、版本與新 id，你核准的內容
  雜湊摘要涵蓋全部內容。
...
這個工具是為 Head of Products 這個角色做的，對象是公司的整個產品組合。設計和決策是我的；程式大部分
由 AI coding agent 在固定的流程下寫出來：Claude Code 一次只實作一個範圍明確的變更，而且先寫測試；
Codex 獨立審查；完整驗證通過；我看過真實 app 才接受。

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/media/pmc-how-it-was-built-dark.gif">
  <img src="docs/media/pmc-how-it-was-built-light.gif" width="880" alt="開發流程：產品負責人設定目標、orchestrator 先寫測試實作一個範圍明確的變更且連續失敗兩次就停、獨立審查、完整驗證、負責人接受後才進主線">
</picture>

兩個真正花了設計功夫的問題：

- 核准一個變更時，重新推導整份快照、當成一個整體比對，而不是只比一個雜湊值。一次稽核發現九類
  紀錄裡有七類重放的是「現在」的狀態，而不是核准當下的狀態；這個修法後來變成每個機制都遵守的規則。
- 還原要在不用 `unsafe` 的前提下換掉正在使用的 SQLite 檔案：兩段式改名、每個階段都寫進控制紀錄，
  啟動時把當機打斷的事做完或退回。

其他的問題、背後的取捨，以及審查抓到了什麼，都連日期寫在下面連結的工程故事裡。
